...
<div class="header">
    <?php
    // Caminho da foto
    $fotoPath = WWW_ROOT . 'uploads/alunos/' . $usuario->matricula . '/imagem_perfil/' . $usuario->foto;

    // Verifica se existe, senão usa padrão
    if (!file_exists($fotoPath)) {
        $fotoPath = WWW_ROOT . 'img/default-user.jpg';
    }

    // Converte em base64
    $tipo = pathinfo($fotoPath, PATHINFO_EXTENSION);
    $dados = file_get_contents($fotoPath);
    $base64 = 'data:image/' . $tipo . ';base64,' . base64_encode($dados);
    ?>
    <img src="<?= $base64 ?>" style="width:100px; height:100px; border-radius:50%;" alt="<?= h($usuario->nome) ?>">
