<?php
/**
 * @var \App\View\AppView $this
 * @var iterable<\App\Model\Entity\Requisito> $requisitos
 */
?>
<div class="requisitos index content">
    <?= $this->Html->link(__('New Requisito'), ['action' => 'add'], ['class' => 'button float-right']) ?>
    <h3><?= __('Requisitos') ?></h3>
    <div class="table-responsive">
        <table>
            <thead>
                <tr>
                    <th><?= $this->Paginator->sort('id') ?></th>
                    <th><?= $this->Paginator->sort('nome') ?></th>
                    <th class="actions"><?= __('Actions') ?></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($requisitos as $requisito): ?>
                <tr>
                    <td><?= $this->Number->format($requisito->id) ?></td>
                    <td><?= h($requisito->nome) ?></td>
                    <td class="actions">
                        <?= $this->Html->link(__('View'), ['action' => 'view', $requisito->id]) ?>
                        <?= $this->Html->link(__('Edit'), ['action' => 'edit', $requisito->id]) ?>
                        <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $requisito->id], ['confirm' => __('Are you sure you want to delete # {0}?', $requisito->id)]) ?>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    </div>
    <div class="paginator">
        <ul class="pagination">
            <?= $this->Paginator->first('<< ' . __('first')) ?>
            <?= $this->Paginator->prev('< ' . __('previous')) ?>
            <?= $this->Paginator->numbers() ?>
            <?= $this->Paginator->next(__('next') . ' >') ?>
            <?= $this->Paginator->last(__('last') . ' >>') ?>
        </ul>
        <p><?= $this->Paginator->counter(__('Page {{page}} of {{pages}}, showing {{current}} record(s) out of {{count}} total')) ?></p>
    </div>
</div>